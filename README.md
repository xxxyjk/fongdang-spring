...
>- 상품 등록 시 펀딩 등록 기한을 통해 목표 펀딩 금액에 달성한 경우 성공적으로 상품 판매를 시작하고 달성하지 못한 경우 펀딩 모금액을 다시 되돌려주는 방식입니다.
 ...
 **화면설명:** 카카오 계정으로 로그인 이후 카카오 서버에 접근 토큰 발급을 요청   
 ...
 **화면설명:** 카카오 서버로부터 받은 사용자의 정보를 조회하여 가입여부 결과에 따라 회원가입과 로그인을 진행   
...
>- 로그아웃 기능 작업 중에 카카오는 기능을 제공하지만 네이버는 제공하지 않아 직접 코드를 짜야했습니다. 그래서 중첩 Ajax를 사용해 서버에 접근 토큰 만료 요청을 위한 url 주소를 보내려고 했지만 CORS정책 문제로 코드 작업이 더뎌짐. 이 후 문제 해결을 위해 OAuth2방식에 대해 더 알아보고 수 차례 디버깅을 시도하며 scribejava를 통해 OAuth2 라이브러리를 사용할 수 있었고 그 결과 토큰 만료 요청으로 인한 로그아웃 기능을 구현함
